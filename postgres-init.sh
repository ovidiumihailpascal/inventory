#!/bin/bash
# PostgreSQL initialization wrapper
# This script is placed in /docker-entrypoint-initdb.d/ and executed by PostgreSQL Docker image
# It substitutes environment variables and then executes the SQL

set -e

# Get the database user from environment or default
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" < /tmp/init-substituted.sql

echo "Database initialization complete for user: $DB_USER"
